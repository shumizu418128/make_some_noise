...
async def on_message(message: discord.Message):
    ...
    if message.content == "s.p msn" or message.content == "s.msn":
        if message.guild.voice_client is None:
            await message.channel.send("接続していません。VCチャンネルに接続してから、もう一度お試しください。")
            return
        audio = discord.PCMVolumeTransformer(discord.FFmpegPCMAudio("msn.mp3"), volume=0.2)
        message.guild.voice_client.play(audio)
        return
    ...
    if message.content == "s.p dismuch" or message.content == "s.dismuch":
        ...
        audio = discord.PCMVolumeTransformer(discord.FFmpegPCMAudio(ran_audio[ran_int]), volume=0.7)
        message.guild.voice_client.play(audio)
        return
